...
from src.verifier.reach import ReachVerifier
...
class TestReachVerifier(unittest.TestCase):
    ...
    def test_action_ltl(self):
        reach_ver = ReachVerifier(self.scenario,
                                  self.config)

        acc = reach_ver.parse_action(LongitudinalAction.ACCELERATE)

        assert acc == ""

        left = reach_ver.parse_action(LateralAction.CHANGE_LEFT)

        assert left == reach_ver.parse_action(LateralAction.CHANGE_LEFT)
